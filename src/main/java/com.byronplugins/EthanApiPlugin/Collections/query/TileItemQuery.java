package com.byronplugins.EthanApiPlugin.Collections.query;
...
public class TileItemQuery {
    ...
    public TileItemQuery nameContainsNoCase(String name) {
        tileItems = tileItems.stream().filter(tileItem ->
        {
            try {
                return EthanApiPlugin.itemDefs.get(tileItem.tileItem.getId()).getName().toLowerCase().contains(name.toLowerCase());
            } catch (ExecutionException e) {
                throw new RuntimeException(e);
            }
        }).collect(Collectors.toList());
